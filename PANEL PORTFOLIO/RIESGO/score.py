...
`calcular_score_cartera` combina una recompensa (Sharpe táctico ajustado) con
penalizaciones por métricas de riesgo incompatibles entre sí (VaR 99% forecast,
CDaR a horizonte, concentración HHI y turnover). Para que sean comparables, cada
métrica se ESTANDARIZA en z-score TRANSVERSAL sobre el conjunto de candidatos y
se combina con los pesos de `_tecnico.PESOS_SCORE`.

    score = z(sharpe) − w_var·z(|VaR99|) − w_cdar·z(|CDaR|)
            − w_hhi·z(HHI) − w_turn·z(turnover)
...
def calcular_score_cartera(
    ...
    z_sharpe, z_var, z_cdar, z_hhi, z_turn = (_z(x) for x in (sharpe, var99, cdar, hhi, turn))
